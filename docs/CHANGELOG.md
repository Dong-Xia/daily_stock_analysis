...
- [新功能] 信号链页与筹码体检页标题旁新增"!"策略说明浮层（样式对齐基本面扫描）：前者涵盖蓝色梯子/DX 信号定义、两条买入规则量化参数、失效线与资金流/520 增强列含义；后者涵盖五档灯语回测依据、算法细节、信号链联动买入语义与体系局限
- [修复] 超跌反弹战法大盘恐慌检查误用 `get_daily_data("000001")`——A股各数据源将其解析为平安银行（深市个股，实测收盘≈11.7 元）而非上证指数，导致指数级阈值套用在单只银行股上；改用东财/新浪指数日线（`sh000001`，实测≈3900 点）
...
- [修复] 板块精选漏斗恒 0 候选并连累链式选股 504：`StockScreenerService.screen()` 的 40s 时间预算在入口起算，成分股抓取（东财被封时兜底路径可达 50-75s）把预算耗尽后 L1/L2 全部跳过；预算改为成分股抓取完成后起算，`hot-sector-chain` 端点与前端超时 300s→600s 容纳冷缓存首轮
- [改进] 新增腾讯批量实时行情（`qt.gtimg.cn` 原生多标的接口）：板块精选 L1 改为批量优先、缺失逐只兜底。逐只路径受防限流随机休眠与 fetcher 级串行锁约束实测 ~6s/只（30 只串行 180s 击穿单板块预算），批量后 30 只一次请求约 3s；链式选股冷缓存 495s→380s、热缓存 32s（≈纯板块间限流间隔），大板块（673 只成分股）从恒 0 候选恢复产出
